...
describe('enterprise-nav-menu', function () {
  before(() => {
    cy.visit('tests/lumen/js/enterprise-global-nav.html');
  });
        ...
        cy.get(SIDENAV_COMPONENT_SELECTOR).within(() => {
          ...
          cy.get('@firstLevelElement')
            ...
            .then((href) => {
              ...
              cy.get('.chi-sidenav__content').click().wait(250);
        ...
        cy.get(SIDENAV_COMPONENT_SELECTOR).within(() => {
          ...
          cy.get('@firstLevelElement')
            ...
            .then((href) => {
              ...
              cy.get('@firstLevelElement').parent().should('have.class', '-active').click().wait(550);

              cy.get('@secondLevelLocation').parents('li').should('have.class', '-active');
  ...
  describe('Check that Menu recovers active state of second and third level items', () => {
    it('Should recover active state when toggling between different lists on 2nd level', () => {
      ...
      cy.get(SIDENAV_COMPONENT_SELECTOR).click(500, 0, { force: true });
      ...
      cy.get('.chi-sidenav__list li').contains('DeepLink').as('switchElement');

      cy.get('@switchElement')
        .should('have.attr', 'href')
        .then((href) => {
          cy.get('' + href).as('drawer2');

          cy.get('@switchElement').click().wait(550);
        });

      cy.get('@drawer2').contains('Option').click().parents('li').should('have.class', '-expanded');

      cy.get('@firstLevelElement').click();
