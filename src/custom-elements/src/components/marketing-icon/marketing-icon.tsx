...
export class MarketingIcon {
  ...
  validateSizeAttribute(newValue: MarketingIconSizes) {
    ...
    if (newValue && VALID_SIZES.indexOf(newValue) === -1) {
      throw new Error(
        'Not valid size (' + newValue + ') for icon ' + this.icon + '. Valid values are ' + validValues + '. '
      );
    }
  }
  ...
  render() {
    return (
      <div class={`chi-marketing-icon ${this.variant} ${this.getClass()} ${this.extraClass ? this.extraClass : ''}`}>
