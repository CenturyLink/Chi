...
const getMutationHandler =
  (handler: Function, forceRender: boolean, element: HTMLElement): MutationCallback =>
 ...
 * @returns
 */
const addMutationObserverDisconnect =
  (disconnectedCallback: Function = () => {}, mutationObserver: MutationObserver): Function =>
  () => {
 ...
 * }
 */
export function addMutationObserver(
  customMutationHandler: MutationCallback = () => {},
  config: MutationObserverInit = { childList: true },
  forceRender = true
) {
  if (!(this.el instanceof HTMLElement)) {
    throw new Error(
      'Mutation observer needs this.el to be defined as the HTMLElement. ' +
  ...
  const mutationObserver = new MutationObserver(
    getMutationHandler(customMutationHandler.bind(this), forceRender, this.el)
  );
