...
export class Drag {
  ...
  private dragMouseDownHandler = (e: MouseEvent) => {
    ...
    this.initialTransformStyling = window.getComputedStyle(this.elementToMove)[
      'transform'
    ];
  ...
  private setPosition = (clientX: number, clientY: number, rect: DOMRect) => {
    ...
    if (rect.top < 0 && this.auxClientCoord > clientY) {
      ...
      this.elementToMove.style.top =
        this.elementToMove.offsetTop - this.positionEnd.y + 'px';
      this.elementToMove.style.left =
        this.elementToMove.offsetLeft - this.positionEnd.x + 'px';
