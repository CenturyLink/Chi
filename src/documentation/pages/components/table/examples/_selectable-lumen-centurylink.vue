<template lang="pug">
  <ComponentExample title="Selectable" id="selectable-lumen-centurylink" :tabs="exampleTabs">
    table.chi-table(slot="example")
      thead
        tr
          th
            .chi-checkbox
              input#portal-chk-1.chi-checkbox__input(type='checkbox')
              label.chi-checkbox__label(for='portal-chk-1')
                .-sr--only Label
          th.-sorted.-ascending
            div Name
          th ID
          th Last Login
      tbody
        tr(v-for="(row, index) in rows" :key="index" :class="{'-active': index === 0}")
          td
            .chi-checkbox
              input.chi-checkbox__input(type='checkbox', :checked="index === 0" :id="`chk-1-0${index}`")
              label.chi-checkbox__label(:for="`chk-1-0${index}`")
                .-sr--only Label
          td {{ row.cell1 }}
          td {{ row.cell2 }}
          td {{ row.cell3 }}
...
export default class SelectableLumenCenturyLink extends Vue {}
</script>
