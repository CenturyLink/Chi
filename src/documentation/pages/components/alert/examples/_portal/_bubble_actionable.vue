<template lang="pug">
  <ComponentExample title="Actionable" titleSize="h4" id="bubble_actionable_portal" :tabs="exampleTabs">
    div(slot='example')
      .-p--1
        chi-alert.-m--2(color='success', icon='circle-check', size='lg', title="Success", closable)
          | This is a large closable success alert
          chi-button(slot="chi-alert__actions", variant='outline', color='primary', uppercase) Secondary action
          chi-button(slot="chi-alert__actions", color='primary', uppercase) Primary action
