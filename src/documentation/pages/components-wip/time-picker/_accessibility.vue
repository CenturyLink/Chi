...
table.chi-table
  ...
  tbody
    tr
      td <code>Tab</code>
      td Focus moves to the hour/minute select and opens the dropdown with hour/minute options.        
    tr
    ...
    tr 
      td <code>Esc</code>
      td Closes the dropdown and returns focus to the hour/minute select trigger.
  ...
  .chi-col.-w-sm--12.-w-md--6.-w-lg--6.-mb--3
    .chi-card.-widget
      ...
      .chi-card__content 
        .chi-card__caption
          ul.-text
            li Keyboard Navigation: Support navigation through hours, minutes, and optionally seconds using arrow keys, and allow selection with the "Enter" key.
            li Use <code>label</code> Elements: Provide descriptive labels for each select component involved in time picking. Ensure the <code>for</code> attribute of the label matches the <code>id</code> of the associated input or select box to support screen readers.
            li Use appropriate ARIA roles and attributes to enhance accessibility. For example, <code>role="listbox"</code> for the select element and <code>role="option"</code> for each option within hour, minute, and second dropdowns.
            ...
            li Test compatibility with various screen readers (like JAWS, NVDA, or VoiceOver) to ensure the time picker component is announced correctly. Options should be clearly read out when navigated.
  ...
  .chi-col.-w-sm--12.-w-md--6.-w-lg--6.-mb--3
    .chi-card.-widget
      ...
      .chi-card__content 
        .chi-card__caption 
          ul.-text
            li Ensure all interactive elements in the time picker, like hour and minute selections and navigation buttons, meet the minimum target size of 44x44 pixels, as recommended by WCAG 2.5.5.
            li Design elements with adequate spacing to prevent accidental activations.
            li Design the time picker with high contrast between text and background colors, ensuring a minimum contrast ratio of 4.5:1 for text.
            ...
            li Maintain consistency in the design of the time picker to provide a cohesive user experience.

h3 Resources
ul.-text
  li
    a(href="https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/time" target="_blank") Mozilla Resources for Developers: 
    | Information about the HTML <code>input-type=time</code>.
  li  
    a(href="https://www.magentaa11y.com/checklist-native/time-picker/" target="_blank") MagentaA11y: 
    | How to test a time picker dialog.
