...
<ComponentExample title="Sizes" id="sizes-portal" :tabs="exampleTabs">
  template(#example-description)
    p.-text Picker groups support the following sizes: <code>-xs</code>, <code>-sm</code>, <code>-md</code>, <code>-lg</code>. The default size is <code>-md</code>.
  template(#example)
    .-d--flex.-flex--column
          ...
          .-py--2
            fieldset
              ...
              .chi-picker-group
                .chi-picker-group__content
                  template(v-for="item in pickers")
                    input.chi-picker__input(type='radio', :name="`example__size_${size}_icon_portal`", :id="`example__size_${size}_icon_${item}_portal`" :checked="item === 1")
