<template lang="pug">
  ComponentExample(title="{breakpoint}" :titleSize="'h4'" :id="id" :noExample="true")
    template(slot="no-example")
      ...
      ul.-text
        li blank - leave blank to apply <code>line-height</code> to all breakpoints.
        li(v-for="breakpoint in breakpoints") <code>{{ breakpoint }}</code> - use to apply <code>line-height</code> to {{ breakpoint }} {{ breakpoint !== 'xl' ? 'and larger' : '' }} breakpoints.
</template>
