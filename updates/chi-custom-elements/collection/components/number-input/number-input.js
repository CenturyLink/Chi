...
export class NumberInput {
     ...
     */
    getInput() {
        return (h("input", { type: "number", ref: (el) => (this._numberInput = el), class: `chi-input ${this.inputstyle ? `-${this.inputstyle}` : ''} ${this.state ? `-${this.state}` : ''}`, disabled: this.disabled, required: this.required, step: this.step, max: this.max, min: this.min, value: this.value, onChange: (ev) => this.handleChange(ev), onInput: (ev) => this.handleInput(ev), onFocus: () => this.chiFocus.emit(), onBlur: () => this.chiBlur.emit(), id: this.el.id ? `${this.el.id}-control` : null }));
    }
    getBaseNumberInput() {
    ...
    render() {
        ...
        if (this.helperMessage) {
            input = (h("div", { key: '125f5c4707673fa60a163b66e23c8e19d4b8ac53', class: "chi-input__wrapper" }, input, this.getHelperMessage()));
        }
        return input;
    }
