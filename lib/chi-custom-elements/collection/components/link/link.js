...
export class Link {
    ...
    render() {
        return (h("a", Object.assign({ key: '16c28637b9f1a689c869cac962c179d3b13d9470', class: `chi-link
          ${this.cta ? '-cta' : ''}
          ${this.disabled ? '-disabled' : ''}
          ${this.size ? `-${this.size}` : ''}
          ${this._hover ? `-hover` : ''}
          ${this.noUnderline ? '-no-underline' : ''}
          ${this.noHoverUnderline ? '-no-hover-underline' : ''}`, href: this.href, hreflang: this.hreflang, target: this.target, rel: this.rel, download: this.download }, (this.alternativeText && { 'aria-label': this.alternativeText })), h("div", { key: 'a8de5eee90b06c96a843d1dfc4a0b4c698cb119f', class: `${this._hasIcon() ? 'chi-link__content' : ''}` }, h("slot", { key: '60387aa4745c79b37dbf7f768a7df0323287b936' }))));
    }
