...
export class Link {
    ...
    render() {
        return (h("a", Object.assign({ key: '05e7db1d10f8b9ac31514af67e49981bae800ac1', class: `chi-link
          ${this.cta ? '-cta' : ''}
          ${this.disabled ? '-disabled' : ''}
          ${this.size ? `-${this.size}` : ''}
          ${this._hover ? `-hover` : ''}
          ${this.noUnderline ? '-no-underline' : ''}
          ${this.noHoverUnderline ? '-no-hover-underline' : ''}`, href: this.href, hreflang: this.hreflang, target: this.target, rel: this.rel, download: this.download }, (this.alternativeText && { 'aria-label': this.alternativeText })), h("div", { key: 'ee52eb269fd6d3a0a98a16995c0682c31705427d', class: `${this._hasIcon() ? 'chi-link__content' : ''}` }, h("slot", { key: 'a93283d5d8110d6b18be6b102bd670c007ff1a01' }))));
    }
