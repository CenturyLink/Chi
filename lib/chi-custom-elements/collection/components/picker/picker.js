...
export class Picker {
    ...
    _getItemLabel(item) {
        ...
        return (h("label", { htmlFor: item.id, id: `${item.id}-label` }, [this.showInput, endSlot, item.description].every((item) => !item)
            ? item.label
    ...
    _getFieldset() {
        return (h("fieldset", null, this._getLabel(), this._getOptions()));
    }
    render() {
        let fieldSet = this._getFieldset();
        if (this.helperMessage) {
            fieldSet = (h("div", { key: '96be5f505123d3bc27956d76ec7fd865769e68f8', class: INPUT_CLASSES.WRAPPER }, fieldSet, h("chi-helper-message", { key: '7b1dca4f49314e79e9843c7105a8b231a7b05ca6', state: this.state }, this.helperMessage)));
        }
        return fieldSet;
    }
