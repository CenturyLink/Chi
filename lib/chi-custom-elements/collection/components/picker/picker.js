...
export class Picker {
    ...
    _getItem(item) {
        const classes = [PICKER_CLASSES.PICKER, this.pill && PILL_CLASS, GENERIC_SIZE_CLASSES[this.size.toUpperCase()]]
            .filter(Boolean)
            .join(' ');
        return (h("div", { class: classes }, h("input", { class: PICKER_CLASSES.INPUT, name: `${this.id}-group`, id: item.id, disabled: item.disabled, checked: item.checked, type: this.type, onClick: (ev) => this._onClick(item, ev) }), this._getItemLabel(item)));
    }
    _getItemLabel(item) {
        const endSlot = this.el.querySelector(`[slot=content-end-${item.id}]`);
        return (h("label", { htmlFor: item.id }, [this.showInput, endSlot, item.description].every((item) => !item)
            ? item.label
            : this._getComplexItemLabelContent(item, endSlot)));
    }
    _getComplexItemLabelContent(item, endSlot) {
        return endSlot ? (h("div", { class: PICKER_CLASSES.CONTENT }, this._getContentStart(item, true), h("div", { class: PICKER_CLASSES.CONTENT_END }, h("slot", { name: `content-end-${item.id}` })))) : (this._getContentStart(item));
    }
    _getContentStart(item, hasEndContent = false) {
        ...
        return description || hasEndContent ? (h("div", { class: this._getLabelContentStartClasses(hasEndContent) }, this._getLabelContent(item), description)) : (this._getLabelContent(item));
    }
    ...
    _getLabelContentStartClasses(hasEndContent) {
        return [
            PICKER_CLASSES.CONTENT_START,
            ...(hasEndContent ? [UTILITY_CLASSES.SIZING.W100, UTILITY_CLASSES.SIZING.WSM65] : []),
        ]
            .filter(Boolean)
            .join(' ');
    }
    ...
    _getFieldset() {
        ...
        return (h("fieldset", null, this._getLabel(), this.pill ? h("div", { class: UTILITY_CLASSES.DISPLAY.FLEX }, options) : options));
    }
    render() {
        let fieldSet = this._getFieldset();
        if (this.helperMessage) {
            fieldSet = (h("div", { key: 'a1df28d64601351eabd59a0cde18f003111634f6', class: INPUT_CLASSES.WRAPPER }, fieldSet, h("chi-helper-message", { key: 'f3477513581ca1b2567e0cbd869f59b9995a4353', state: this.state }, this.helperMessage)));
        }
        return fieldSet;
    }
