...
export class DatePicker {
    ...
    render() {
        ...
        const date = (h("chi-date", { key: '04d1c4792882098d12a6d10aefe4e67b8e418555', min: this.min, max: this.max, locale: this.locale, value: chiDateValue, format: this.format, "excluded-weekdays": this.excludedWeekdays, "excluded-dates": this.excludedDates, multiple: this.multiple, onChiDateChange: (ev) => this.handleDateChange(ev) }));
        ...
        const chiPopover = (h("chi-popover", { key: '9bc273922a0773bf6dd911ea2119276c4f33bc48', id: "example-4-be-popover", position: this.mode === 'datetime' ? 'bottom-start' : 'bottom', reference: `#${this._uuid}-control`, "prevent-auto-hide": true, active: this.active }, popoverContent));
        const inputClasses = `chi-input
      ${this.active ? '-focus' : ''}
      ${this.state ? '-' + this.state : ''}
    `;
        const helperMessage = this.helperMessage && (h("chi-helper-message", { key: '44f43b1921ebc6d6cf6fe5e9eb0bcfcc75a8ff36', state: this.state }, this.helperMessage));
        return [
            // TODO: This input should be chi-input in the future and will pass through
            // some of its configuration attributes.
            h("div", { class: `
        ${this.disabled ? '-disabled' : ''}
        ${this.mode === 'datetime' ? '-time' : ''}`, key: this._uuid }, h("div", { key: 'a3e3fcb58146f0715894041e2f63614c1679350b', class: "chi-input__wrapper -icon--right" }, h("input", { key: 'dbf0ad7b709a72ac9d67614c895b323bcf3c2321', id: `${this._uuid}-control`, class: inputClasses, type: "text", placeholder: this._getPlaceholder(), ref: (el) => (this._input = el), value: this.value && this.multiple ? String(this.value).replace(/,/g, ', ') : this.value, onChange: () => {
                    this._checkDate();
                }, disabled: this.disabled }), h("chi-icon", { key: 'ee3f987fa0502c1a0e0e5729429a31d12d836952', icon: "date" })), helperMessage, !this.disabled && chiPopover),
        ];
    }
