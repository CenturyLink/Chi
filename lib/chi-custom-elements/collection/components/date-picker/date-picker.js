...
export class DatePicker {
    ...
    render() {
        ...
        const date = (h("chi-date", { key: 'd92c449e85fad7d534b5d598cc8c79841fc2369d', min: this.min, max: this.max, locale: this.locale, value: chiDateValue, format: this.format, "excluded-weekdays": this.excludedWeekdays, "excluded-dates": this.excludedDates, multiple: this.multiple, onChiDateChange: (ev) => this.handleDateChange(ev) }));
        ...
        const chiPopover = (h("chi-popover", { key: 'b1849bc2e0db5841f1106a42681949c7916ea084', id: "example-4-be-popover", position: this.mode === 'datetime' ? 'bottom-start' : 'bottom', reference: `#${this._uuid}-control`, "prevent-auto-hide": true, active: this.active }, popoverContent));
        const inputClasses = `chi-input
      ${this.active ? '-focus' : ''}
      ${this.state ? '-' + this.state : ''}
    `;
        const helperMessage = this.helperMessage && (h("chi-helper-message", { key: 'b95f815f64fcec6c5055092333267241cca24303', state: this.state }, this.helperMessage));
        return [
            // TODO: This input should be chi-input in the future and will pass through
            // some of its configuration attributes.
            h("div", { class: `
        ${this.disabled ? '-disabled' : ''}
        ${this.mode === 'datetime' ? '-time' : ''}`, key: this._uuid }, h("div", { key: 'c2e6bf6e835ef6ce2b74a6b36e4b83e52256c5d0', class: "chi-input__wrapper -icon--right" }, h("input", { key: 'd08b9b9cbd7ba13e6766614412976584b13d63ed', id: `${this._uuid}-control`, class: inputClasses, type: "text", placeholder: this.mode === 'datetime' ? `${this.format}, --:-- --` : this.format, ref: (el) => (this._input = el), value: this.value && this.multiple ? String(this.value).replace(/,/g, ', ') : this.value, onChange: () => {
                    this._checkDate();
                }, disabled: this.disabled }), h("chi-icon", { key: '05e2442db34bbcfa8e4f69158ad6b0513691758a', icon: "date" })), helperMessage, !this.disabled && chiPopover),
        ];
    }
