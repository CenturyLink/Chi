...
export class DatePicker {
    ...
    render() {
        ...
        const date = (h("chi-date", { key: 'd44d58d71bc64069180367278f7cd145afe87484', min: this.min, max: this.max, locale: this.locale, value: chiDateValue, format: this.format, "excluded-weekdays": this.excludedWeekdays, "excluded-dates": this.excludedDates, multiple: this.multiple, onChiDateChange: (ev) => this.handleDateChange(ev) }));
        ...
        const chiPopover = (h("chi-popover", { key: '7f4404cdf3468c8ea6acb1a3eda141336d06572b', id: "example-4-be-popover", position: this.mode === 'datetime' ? 'bottom-start' : 'bottom', reference: `#${this._uuid}-control`, "prevent-auto-hide": true, active: this.active }, popoverContent));
        const inputClasses = `chi-input
      ${this.active ? '-focus' : ''}
      ${this.state ? '-' + this.state : ''}
    `;
        const helperMessage = this.helperMessage && (h("chi-helper-message", { key: '05a58299adbb36f7641f7fe7693466bada0c354b', state: this.state }, this.helperMessage));
        return [
            // TODO: This input should be chi-input in the future and will pass through
            // some of its configuration attributes.
            h("div", { class: `
        ${this.disabled ? '-disabled' : ''}
        ${this.mode === 'datetime' ? '-time' : ''}`, key: this._uuid }, h("div", { key: 'd6d77bceacd5227ecc32baea651f23c42a9bc69e', class: "chi-input__wrapper -icon--right" }, h("input", { key: '6c39f6e3b2fa32b2e659f2bd77ccda037bf163c8', id: `${this._uuid}-control`, class: inputClasses, type: "text", placeholder: this.mode === 'datetime' ? `${this.format}, --:-- --` : this.format, ref: (el) => (this._input = el), value: this.value && this.multiple ? String(this.value).replace(/,/g, ', ') : this.value, onChange: () => {
                    this._checkDate();
                }, disabled: this.disabled }), h("chi-icon", { key: '95231dc9dca95eb0112ae8a04ffc88059f2c823b', icon: "date" })), helperMessage, !this.disabled && chiPopover),
        ];
    }
