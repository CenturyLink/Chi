...
export class DatePicker {
    ...
    render() {
        ...
        const date = (h("chi-date", { key: '0ca6e5f914878750d0db57b956a29077d82cc1ee', min: this.min, max: this.max, locale: this.locale, value: chiDateValue, format: this.format, "excluded-weekdays": this.excludedWeekdays, "excluded-dates": this.excludedDates, multiple: this.multiple, onChiDateChange: (ev) => this.handleDateChange(ev) }));
        ...
        const chiPopover = (h("chi-popover", { key: 'f7338a6530fef05a769d23c9c379b9fd56c64c41', id: "example-4-be-popover", position: this.mode === 'datetime' ? 'bottom-start' : 'bottom', reference: `#${this._uuid}-control`, "prevent-auto-hide": true, active: this.active }, popoverContent));
        const inputClasses = `chi-input
      ${this.active ? '-focus' : ''}
      ${this.state ? '-' + this.state : ''}
    `;
        const helperMessage = this.helperMessage && (h("chi-helper-message", { key: 'a311c7a313ae6ee105d061fa2b90fb599638f985', state: this.state }, this.helperMessage));
        return [
            // TODO: This input should be chi-input in the future and will pass through
            // some of its configuration attributes.
            h("div", { class: `
        ${this.disabled ? '-disabled' : ''}
        ${this.mode === 'datetime' ? '-time' : ''}`, key: this._uuid }, h("div", { key: 'ed7e4315d775a41427b4951784a9418e45ee828e', class: "chi-input__wrapper -icon--right" }, h("input", { key: 'e881022e5e1e961566cd4fc6c054cdc453af6b8f', id: `${this._uuid}-control`, class: inputClasses, type: "text", placeholder: this.mode === 'datetime' ? `${this.format}, --:-- --` : this.format, ref: (el) => (this._input = el), value: this.value && this.multiple ? String(this.value).replace(/,/g, ', ') : this.value, onChange: () => {
                    this._checkDate();
                }, disabled: this.disabled }), h("chi-icon", { key: '239bfa2e9656285fcaa9235652adba4fe8fef0fb', icon: "date" })), helperMessage, !this.disabled && chiPopover),
        ];
    }
