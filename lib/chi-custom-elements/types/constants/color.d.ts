export declare const TEXT_COLORS: string[];
export declare const SEMANTIC_COLORS: string[];
export declare const ICON_COLORS: readonly ["primary", "success", "warning", "danger", "light", "dark", "muted", "info", "grey", "secondary", "navy", "orange"];
export type IconColors = (typeof ICON_COLORS)[number];
export declare const ALERT_COLORS: readonly ["success", "warning", "danger", "info", "muted"];
export type AlertColors = (typeof ALERT_COLORS)[number];
