...
const DatePicker = class {
    ...
    render() {
        ...
        const date = (index.h("chi-date", { key: '57e4928966e9e2f0c9fc024e51d5721c0b58e005', min: this.min, max: this.max, locale: this.locale, value: chiDateValue, format: this.format, "excluded-weekdays": this.excludedWeekdays, "excluded-dates": this.excludedDates, multiple: this.multiple, onChiDateChange: (ev) => this.handleDateChange(ev) }));
        ...
        const chiPopover = (index.h("chi-popover", { key: '2e5f4f441f8c1b0758e516f00782ba35357342ec', id: "example-4-be-popover", position: this.mode === 'datetime' ? 'bottom-start' : 'bottom', reference: `#${this._uuid}-control`, "prevent-auto-hide": true, active: this.active }, popoverContent));
        const inputClasses = `chi-input
      ${this.active ? '-focus' : ''}
      ${this.state ? '-' + this.state : ''}
    `;
        const helperMessage = this.helperMessage && (index.h("chi-helper-message", { key: 'eec0f83fcd03b01ba1c9869694853a964cad4b80', state: this.state }, this.helperMessage));
        return [
            // TODO: This input should be chi-input in the future and will pass through
            // some of its configuration attributes.
            index.h("div", { class: `
        ${this.disabled ? '-disabled' : ''}
        ${this.mode === 'datetime' ? '-time' : ''}`, key: this._uuid }, index.h("div", { key: '562be008fe486421c6ff48b84a07b0931ae1c9cc', class: "chi-input__wrapper -icon--right" }, index.h("input", { key: '46f63cf2a64895917ded9854f6336fc158c2e0f6', id: `${this._uuid}-control`, class: inputClasses, type: "text", placeholder: this._getPlaceholder(), ref: (el) => (this._input = el), value: this.value && this.multiple ? String(this.value).replace(/,/g, ', ') : this.value, onChange: () => {
                    this._checkDate();
                }, disabled: this.disabled }), index.h("chi-icon", { key: '00d73873043e691f8d5b10f56a81ebb992ddc631', icon: "date" })), helperMessage, !this.disabled && chiPopover),
        ];
    }
