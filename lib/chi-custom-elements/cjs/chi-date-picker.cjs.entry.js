...
const DatePicker = class {
    ...
    render() {
        ...
        const date = (index.h("chi-date", { key: '55eb5df5281c6c9adc93f16233b63f2642301633', min: this.min, max: this.max, locale: this.locale, value: chiDateValue, format: this.format, "excluded-weekdays": this.excludedWeekdays, "excluded-dates": this.excludedDates, multiple: this.multiple, onChiDateChange: (ev) => this.handleDateChange(ev) }));
        ...
        const chiPopover = (index.h("chi-popover", { key: '465cd801a87e0fd83b7498b8f5ecadd5bb3b516f', id: "example-4-be-popover", position: this.mode === 'datetime' ? 'bottom-start' : 'bottom', reference: `#${this._uuid}-control`, "prevent-auto-hide": true, active: this.active }, popoverContent));
        const inputClasses = `chi-input
      ${this.active ? '-focus' : ''}
      ${this.state ? '-' + this.state : ''}
    `;
        const helperMessage = this.helperMessage && (index.h("chi-helper-message", { key: 'cbc3b59cb00ead4b1605c736cdfcbddc5403228d', state: this.state }, this.helperMessage));
        return [
            // TODO: This input should be chi-input in the future and will pass through
            // some of its configuration attributes.
            index.h("div", { class: `
        ${this.disabled ? '-disabled' : ''}
        ${this.mode === 'datetime' ? '-time' : ''}`, key: this._uuid }, index.h("div", { key: '16ddd7deb9c0eadb913807cb0ccf62ab26a7bffe', class: "chi-input__wrapper -icon--right" }, index.h("input", { key: '5e88158bf6615c086abd2c13f7a9fe95817bfca0', id: `${this._uuid}-control`, class: inputClasses, type: "text", placeholder: this._getPlaceholder(), ref: (el) => (this._input = el), value: this.value && this.multiple ? String(this.value).replace(/,/g, ', ') : this.value, onChange: () => {
                    this._checkDate();
                }, disabled: this.disabled }), index.h("chi-icon", { key: '96b8b19fcd65cc53f29e779c83be2c3f81082181', icon: "date" })), helperMessage, !this.disabled && chiPopover),
        ];
    }
