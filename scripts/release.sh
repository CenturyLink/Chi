...
grep -rl "$CURRENT_VERSION" --exclude-dir={lib,src,dist,node_modules,tests,backstop_data,reports,.git} --exclude={package-lock.json,CHANGELOG.md} . | while read -r file; do
