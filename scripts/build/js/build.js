...
async function buildJS() {
  ...
  try {
    execSync(`${isWindows ? 'cross-env' : ''} JS=iife vite build --c vite-js.config.ts`, { stdio: 'ignore' });
  ...
  try {
    execSync(`${isWindows ? 'cross-env' : ''} JS=amd vite build --c vite-js.config.ts`, { stdio: 'ignore' });
